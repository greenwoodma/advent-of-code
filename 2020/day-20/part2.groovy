...
for (int i = 0 ; i < tiles.size()-1 ; ++i) {
	...
	for (int j = i + 1 ; j < tiles.size() ; ++j) {
		...
		if (align) {
			Set<String> ids = counts.getOrDefault(ti.id,new HashSet<String>());
			ids.add(tj.id);
			counts.put(ti.id,ids);

			ids = counts.getOrDefault(tj.id,new HashSet<String>());
			ids.add(ti.id);
			counts.put(tj.id,ids);
		}
...
for (Map.Entry<String,Set<String>> tile : counts.entrySet()) {
	if (tile.getValue().size() <= 2) {
