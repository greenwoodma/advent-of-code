...
String test1 = "1 + 2 * 3 + 4 * 5 + 6"; //231
String test2 = "1 + (2 * 3) + (4 * (5 + 6))"; //51
String test3 = "((2 + 4 * 9) * (6 + 9 * 8 + 6) + 6) + 2 + 4 * 2"; //23340
...
public long doMaths(String input) {
	...
	while (input.indexOf("+") != -1) {
		...
		while (m.find()) {
			m.appendReplacement(simpler, (Long.valueOf(m.group(1))+Long.valueOf(m.group(2))).toString());
		}
