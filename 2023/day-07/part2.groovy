...
class Hand implements Comparable {
	...
	public Hand(String desc) {
		...
		if (counts.size() > 1 && counts.containsKey((char)'J')) {
			//System.out.println("add joker count to " + most);
			counts.put(most, counts.get(most)+counts.remove((char)'J'));
		}
